...
In most cases, I recommend a **hybrid approach** 👍. Many applications use message queues for processing tasks but also log the status of each task in the database.

Imagine a scenario where user uploads a document for some background processing like data extraction. You can imagine the following flow:
